...
AlarmClockController::AlarmClockController(Controllers::DateTime& dateTimeController) : dateTimeController {dateTimeController} {
}

namespace {
  void SetOffAlarm(TimerHandle_t xTimer) {
    auto controller = static_cast<Pinetime::Controllers::AlarmClockController*>(pvTimerGetTimerID(xTimer));
    controller->SetOffAlarmNow();
  }
}

void AlarmClockController::Init(System::SystemTask* systemTask) {
  this->systemTask = systemTask;
  alarmTimer = xTimerCreate("Alarm", 1, pdFALSE, this, SetOffAlarm);
}

void AlarmClockController::SetAlarmTime(uint8_t alarmHr, uint8_t alarmMin) {
  hours = alarmHr;
  minutes = alarmMin;
}

void AlarmClockController::ScheduleAlarm() {
